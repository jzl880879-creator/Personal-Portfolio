...
export default function ValidationPage(){
  ...
  return <main className="decision-page validation-page" id="validation-top">
    ...
    <header className="decision-hero validation-hero"><div className="decision-hero-media validation-hero-media" aria-hidden="true">{validationHeroVideos.slice(0,mediaMode==='full'?validationHeroVideos.length:1).map((video,index)=><video className={index===heroShot?'is-active':''} autoPlay muted loop playsInline preload={index===0?'metadata':'none'} poster={index===0?'/home-product-design-prototype-poster.jpg':undefined} key={video}><source src={video} type="video/mp4"/></video>)}</div><div className="decision-veil"/><div className="decision-shell decision-hero-copy"><p>03 / DESIGN & VALIDATION</p><h1><span>设计</span><br/>与验证</h1><blockquote>设计不是提前给出答案，<br/>而是把判断转化为可验证的假设，<br/>再用证据推动方案收敛。</blockquote></div></header>
