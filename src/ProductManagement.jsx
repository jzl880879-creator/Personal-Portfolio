...
export default function ProductManagement(){
  ...
  return <main className="medical-page product-management-page" id="top">
    ...
    <section className="medical-project-selector" aria-label="产品立项项目导航" onClick={closeFromBlank}>
      ...
      <div className="selector-slats medical-shell">{projects.map((project,index)=><a className={`selector-slat ${activeProject===index?'is-active':''}`} href={`#pm-project-${project.no}`} onClick={(event)=>openProject(event,index)} key={project.no}><img src={project.cover} alt=""/><div className="selector-shade"/><div className="selector-number">{project.no}</div><div className="selector-copy"><span>{project.en}</span><h2>{project.title}</h2><p>打开查看产品详情 <ArrowUpRight size={16}/></p></div></a>)}</div>
    </section>
    {active&&<section className="medical-render-viewer" ref={galleryRef} id="product-management-gallery"><div className="render-viewer-layout medical-shell"><aside className="render-project-nav medical-floating-nav" ref={projectNavRef}><div className="medical-floating-nav__inner"><nav>{projects.map((project,index)=><button className={activeProject===index?'active':''} onClick={()=>setActiveProject(index)} key={project.no}><i>{project.no}</i><b>{project.title}</b></button>)}</nav><a href="#top">返回页面顶部 <ArrowUpRight size={14}/></a></div></aside><div className="render-project-content" id={`pm-project-${active.no}`}><header><span>{active.no} / 04</span><p>{active.en}</p><h2>{active.title}</h2><div>{active.intro}</div></header><div className="render-image-wall">{active.images.map((src,index)=><figure key={src}><img src={src} alt={`${active.title} 项目页面 ${index+1}`} loading={index===0?'eager':'lazy'}/><figcaption>{String(index+1).padStart(2,'0')} / {String(active.images.length).padStart(2,'0')}</figcaption></figure>)}</div></div></div></section>}
