...
export default function DecisionPage(){
  ...
  return <main className="decision-page" id="decision-top">
    ...
    <header className="decision-hero">
      <div className="decision-hero-media" aria-hidden="true">{heroVideos.map((video,index)=><video className={index===heroShot?'is-active':''} autoPlay muted loop playsInline preload={index===0?'auto':'metadata'} key={video}><source src={video} type="video/mp4"/></video>)}</div><div className="decision-veil"/>
    ...
    <section className="decision-practice" id="practice"><div className="decision-shell"><header className="decision-section-head decision-reveal"><span>02 / EXPERIENCE</span><h2>不同项目，<br/>对应不同的优先级。</h2></header><div className="decision-case-list">{cases.map((item,index)=><article className="decision-reveal" key={item.type}><figure><img src={['/decision/case-electronics.jpg','/decision/case-medical.jpg','/decision/case-product-roadmap.jpg'][index]} alt={item.title}/><span>0{index+1}</span></figure><div><p>{item.type}</p><h3>{item.title}</h3><strong>{item.body}</strong></div></article>)}</div></div></section>
